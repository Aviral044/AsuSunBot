...
# The models you are testing (Your Baseline)
TEST_LLM_NAME = "openai/gpt-oss-120b"
TEST_EMBED_NAME = "BAAI/bge-small-en-v1.5"

# Ragas needs a strong LLM to grade the answers
JUDGE_LLM_NAME = "llama-3.3-70b-versatile"

def setup_baseline_bot(api_key):
    """Recreates your exact LlamaIndex setup using a specific API key."""
...
def run_evaluation():
    ...
    # Fix for Groq constraints
    answer_relevancy.strictness = 1
    ragas_config = RunConfig(max_workers=1, max_retries=3)

    all_evaluation_results = []

    # Loop through the questions one by one to evaluate and handle limits safely
    for i in tqdm(range(len(questions)), desc="Grading with Ragas", unit="question"):
        ...
        while not success:
            try:
                # Initialize the Judge LLM with the CURRENT active key
                judge_llm = ChatGroq(
